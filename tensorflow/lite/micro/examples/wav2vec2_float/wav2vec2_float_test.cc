...
#include "tensorflow/lite/micro/examples/wav2vec2_float/wav2vec2_dynamic.h"
...
//   tflite::AllOpsResolver resolver;
  ...
  TF_LITE_MICRO_EXPECT_EQ(32, output->dims->data[1]);
  ...
  for (int i = 0; i<81; ++i){
    ...
    for (int j=0; j<32; ++j){
        MicroPrintf("%g", (double)output_data[i*32+j]);
    }
    MicroPrintf("\n");
  }
